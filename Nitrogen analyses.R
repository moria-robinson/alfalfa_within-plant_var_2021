...
data.pp <- do.call(data.frame, aggregate(n ~ plant_pop + dom_wild + plant.volume_centered + repro.stage + repro.stage.simp + percent_FAL + percent_HEM + percent_CAER, data=data.pp_sub, function(x) c(mean = mean(x), sd = sd(x), N = length(x)))); head(data.pp); nrow(data.pp) # 60 - one measure of mean and sd per plant
...
# across all leaves of all relative age classes); keep relevant plant-level covariates
data.ps <- do.call(data.frame, aggregate(n ~ plant_pop + dom_wild + plant.volume_centered + repro.stage + repro.stage.simp + percent_FAL + percent_HEM + percent_CAER + leaf_stage, data=data.ps_sub, function(x) c(mean = mean(x), sd = sd(x), N = length(x)))); head(data.ps); nrow(data.ps) # 180 - 3 measures of mean and sd per plant (one per relative leaf age class)
names(data.ps) <- c("plant_pop", "dom_wild", "plant.volume_centered", "repro.stage", "repro.stage.simp", "percent_FAL","percent_HEM","percent_CAER","leaf_stage", "mean", "sd", "N")
...
dredge(m1.4) # Top models (within 2AIC) have mean and dom_wild and plant size; no interactions of interest
topmodel.pp <- glmmTMB(sd ~ mean + dom_wild + plant.volume_centered, data=data.pp, na.action = "na.fail")
...
topmodel.pp_nomean <- glmmTMB(sd ~ dom_wild + plant.volume_centered, data=data.pp, na.action = "na.fail")
...
dredge(m4.1) # Top models (by > 2AIC) has dom_wild + percent_FAL, no plant size
...
dredge(m5.4) # top models (< 2 AIC) have all the main effects, no intxns
topmodel.ps <- glmmTMB(sd ~ dom_wild + leaf_stage + mean + plant.volume_centered + (1|plant_pop), data=data.ps) 
...
topmodel.ps_nomean <- glmmTMB(sd ~ dom_wild + leaf_stage + plant.volume_centered + (1|plant_pop), data=data.ps) 
...
fixef(topmodel.ps_nomean)$cond["dom_wilddomestic"] # confirm how to call the right component of the model object
b1 <- lme4::bootMer(topmodel.ps_nomean, FUN=function(x) fixef(x)$cond["dom_wilddomestic"], nsim=500, .progress="txt")
boot <- boot.ci(b1,type="perc")

# Pull out the effect (beta) from the model
beta <- as.numeric(fixef(topmodel.ps_nomean)$cond["dom_wilddomestic"])
# Pull out the bootstrapped 95% CI around that beta. I need to get this from the "percent" part of the bootstrapping output. The items within aren't named, which is annoying. From bootMer, below "These latter four components will be matrices with 5 columns, the first column containing the level, the next two containing the indices of the order statistics used in the calculations and the final two the calculated endpoints themselves." So I need to pull the latter two.
boot; str(boot); class(boot)
out <- data.frame(boot$percent); out # so the last two components are the upper and lower CIs, respecively
beta.CI_upper.boot <- as.numeric(out[length(out)]); beta.CI_upper.boot 
...
topmodel.mean <- glmmTMB(mean ~ dom_wild*leaf_stage + plant.volume_centered + percent_FAL + (1|plant_pop), data=data.ps, na.action = "na.fail")
DHARMa::testResiduals(DHARMa::simulateResiduals(topmodel.mean))
...
ggplot(data=n, aes(x=leaf_stage, y=percent.change, group=hypothesis)) +
  ...
  theme_bw()+
  geom_text(aes(label = sig.asterisk, y=max(n$percent.change.upper)+25), size=5, color="gray32", position=position_dodge(width=.5), angle=30) +
