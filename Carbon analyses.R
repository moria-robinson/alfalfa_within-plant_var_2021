...
data.pp <- do.call(data.frame, aggregate(c ~ plant_pop + dom_wild + plant.volume_centered + repro.stage + repro.stage.simp + percent_FAL + percent_HEM + percent_CAER, data=data.pp_sub, function(x) c(mean = mean(x), sd = sd(x), N = length(x)))); head(data.pp); nrow(data.pp) # 60 - one measure of mean and sd per plant
...
dredge(m1.4) # Top model has percent_FAL; no intxn. Keep (insig) main FX + percent FAL
topmodel.pp <- glmmTMB(sd ~ dom_wild + mean + percent_FAL, data=data.pp, na.action = "na.fail")
...
dredge(m4.1) # Top models have no covariates
...
AICctab(m5.1, m5.2, m5.3) # m5.2 and m5.3 are the same; use plant_pop
...
dredge(m5.4) # top models (< 2 AIC) have mean and plant size and percent_FAL. Wow. Still want to visualize non-sig FX; so keep in (non-sig) dom_wild & leaf stage. But no intxns.
topmodel.ps <- glmmTMB(sd ~ dom_wild + leaf_stage + mean + plant.volume_centered + percent_FAL + (1|plant_pop), data=data.ps) 
...
topmodel.ps_nomean <- glmmTMB(sd ~ dom_wild + leaf_stage + percent_FAL + (1|plant_pop), data=data.ps) 
...
dredge(m6.4) # Top model has dom, leaf stage, and interaction but no plant size
...
ggplot(data=c, aes(x=leaf_stage, y=percent.change, group=hypothesis)) +
  ...
  theme_bw()+
  geom_text(aes(label = sig.asterisk, y=max(c$percent.change.upper)+25), size=5, color="gray32", position=position_dodge(width=.5), angle=30) +
