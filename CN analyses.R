...
#  Per-plant  #
#-------------#
data.pp_sub <- subset(data, subset.whole_plant_var == "KEEP"); nrow(data); nrow(data.pp_sub) # see wrangle script; this removes plants with not all leaf age classes represented (due to loss of some samples in transit)
# across all leaves of all relative age classes); keep relevant plant-level covariates
data.pp <- do.call(data.frame, aggregate(cn ~ plant_pop + dom_wild + plant.volume_centered + repro.stage + repro.stage.simp + percent_FAL + percent_HEM + percent_CAER, data=data.pp_sub, function(x) c(mean = mean(x), sd = sd(x), N = length(x)))); head(data.pp); nrow(data.pp) # 60 - one measure of mean and sd per plant
...
#-------------------------------#
data.ps_sub <- subset(data, subset.leaf_stage_var == "KEEP"); nrow(data); nrow(data.ps_sub) 
# across all leaves of all relative age classes); keep relevant plant-level covariates
data.ps <- do.call(data.frame, aggregate(cn ~ plant_pop + dom_wild + plant.volume_centered + repro.stage + repro.stage.simp + leaf_stage + percent_FAL + percent_HEM + percent_CAER, data=data.ps_sub, function(x) c(mean = mean(x), sd = sd(x), N = length(x)))); head(data.ps); nrow(data.ps) # 180 - 3 measures of mean and sd per plant (one per relative leaf age class)
...
dredge(m1.4) # Top models (within 2AIC) have mean and dom_wild and no plant size; also mean* dom_wild intxn
topmodel.pp <- glmmTMB(sd ~ dom_wild*mean, data=data.pp, na.action = "na.fail")
...
#model.ests <- data.frame(ggpredict(topmodel.pp, terms=c("dom_wild", "mean [9.92]"))) # @ low CN content, no diff
model.ests <- data.frame(ggpredict(topmodel.pp, terms=c("dom_wild", "mean [13.87]"))) # Use mean CN content
#model.ests <- data.frame(ggpredict(topmodel.pp, terms=c("dom_wild", "mean [18.71]"))) # At high CN content, biggest diff
...
##### E) Bootstrap CI 
# Here, I'll extract the effect of domestication at the mean N content, due to the significant interaction 
...
# For example - 
fixef(topmodel.pp) # the same; makes sense
emmeans(topmodel.pp, specs = trt.vs.ctrl ~ dom_wild|mean) # @ the mean level of CN

contrasts <- function(x){
  em <- emmeans(x, specs = trt.vs.ctrl ~ dom_wild|mean) # will pull out the estimate @ the mean 
...
# Bootstrapping...
b1 <- lme4::bootMer(topmodel.pp, FUN=function(x) contrasts(x), nsim=500, .progress="txt")
...
AICctab(m3.1, m3.2)
# < 2AIC difference
# compare repro.stage with uninformative dummy var, decide to keep or not in model
...
dredge(m4.1) # Top models (by > 2AIC) has only dom_wild, no plant size
topmodel.mean <- glmmTMB(mean ~ dom_wild + percent_FAL, data=data.pp, na.action = "na.fail")
...
##### A) Tweak ranFX structure
data.ps$plant_pop_dummy <- as.factor(sample(unique(data.ps$plant_pop), replace=T, nrow(data.ps))); head(data.ps)
...
m5.2_dummy <- glmmTMB(sd ~ dom_wild + leaf_stage + mean + plant.volume_centered + (1|plant_pop_dummy), data=data.ps)
AICctab(m5.1, m5.2, m5.3,m5.2_dummy) # random plant_pop same as plant_pop; no random effect needed

##### B) Tweak fixedFX structure
m5.4 <- glmmTMB(sd ~ dom_wild*leaf_stage*mean + plant.volume_centered + percent_FAL + percent_CAER, data=data.ps)
dredge(m5.4) # top models (< 2 AIC) have 3-way intxn, no plant size
topmodel.ps <- glmmTMB(sd ~ dom_wild*leaf_stage*mean, data=data.ps) 
DHARMa::testResiduals(DHARMa::simulateResiduals(topmodel.ps))

##### C) Pull out predicted means for wild & domestic + model nobs + df
mean(data.ps$mean); range(data.ps$mean)
#model.ests <- data.frame(ggpredict(topmodel.ps, terms=c("dom_wild", "leaf_stage", "mean [7.73]"))) # @ low CN content, more of a diff for young leaves
model.ests <- data.frame(ggpredict(topmodel.ps, terms=c("dom_wild", "leaf_stage", "mean [13.84]"))) # Use mean CN content (shift is most pronounced for middle leaves)
#model.ests <- data.frame(ggpredict(topmodel.ps, terms=c("dom_wild", "leaf_stage", "mean [21.44]"))) # At high CN content, biggest diff (for middle leaves)
ggplot(model.ests, aes(x=x, y=predicted)) +
  geom_point() +
  geom_errorbar(aes(ymin=conf.low, ymax=conf.high), size=.5, width=.2, color="black") +
  facet_grid(~group)
...
##### D) Calculate / append significance 
sig0 <- data.frame(pairs(emmeans(topmodel.ps, ~ dom_wild|leaf_stage*mean))) # post-hoc test 
names(sig0)[names(sig0) == 't.ratio'] <- 'statistic'
names(sig0)[names(sig0) == 'df'] <- 'statistic.df'
sig0$test.statistic <- "t.ratio"
...
##### E) Bootstrap CI 
# Here, I'll extract the effect of domestication at the mean CN content, due to the significant interaction 
# (Used this great post as a resource: https://www.r-bloggers.com/bootstrapping-follow-up-contrasts-for-within-subject-anovas-part-2/)
# Also used this helpful page to re-level the order of comparisons in emmeans (for some reason it always considers domestic the 'control' group, so estimates are negative rather than positive). 
# https://aosmith.rbind.io/2019/03/25/getting-started-with-emmeans/
# For example - 
normal <- pairs(emmeans(topmodel.ps, ~ dom_wild|leaf_stage*mean)); normal # estimates reversed
#versus
em <- emmeans(topmodel.ps, specs = trt.vs.ctrl ~ dom_wild|leaf_stage*mean); em$contrasts # estimates are in the correct order, positive when an increase with domestication - yay!

contrasts <- function(x){
  em <- emmeans(x, specs = trt.vs.ctrl ~ dom_wild|leaf_stage*mean)
  con <- data.frame(em$contrasts)
  ests <- con$estimate
...
# Bootstrapping...
b1 <- lme4::bootMer(topmodel.ps, FUN=function(x) contrasts(x), nsim=500, .progress="txt")
summ <- data.frame(summary(b1)); summ
names(summ)[names(summ) == 'original'] <- 'beta'
...
##### A) Tweak ranFX structure NA
##### B) Tweak fixed FX structure
topmodel.ps_nomean <- glmmTMB(sd ~ dom_wild*leaf_stage, data=data.ps) 
...
dredge(m6.4) # Top models has interaction but no plant size
topmodel.mean <- glmmTMB(mean ~ dom_wild*leaf_stage + percent_FAL + (1|plant_pop), data=data.ps, na.action = "na.fail")
DHARMa::testResiduals(DHARMa::simulateResiduals(topmodel.mean))
...
# https://aosmith.rbind.io/2019/03/25/getting-started-with-emmeans/
# For example - 
normal <- pairs(emmeans(topmodel.mean, ~ dom_wild|leaf_stage)); normal # estimates always negative
#versus
em <- emmeans(topmodel.mean, specs = trt.vs.ctrl ~ dom_wild|leaf_stage); em$contrasts # estimates are in the correct order, contrast order reversed - yay!
...
ggplot(data=cn, aes(x=leaf_stage, y=percent.change, group=hypothesis)) +
  ...
  theme_bw()+
  geom_text(aes(label = sig.asterisk, y=max(cn$percent.change.upper)+25), size=5, color="gray32", position=position_dodge(width=.5), angle=30) +
