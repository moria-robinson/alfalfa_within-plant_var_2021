#######################################
# Specific Leaf Area (SLA) - analyses #
#######################################
# Goals: 
...
data.pp <- do.call(data.frame, aggregate(SLA ~ plant_pop + dom_wild + plant.volume_centered + repro.stage + repro.stage.simp + percent_FAL + percent_HEM + percent_CAER, data=data, function(x) c(mean = mean(x), sd = sd(x), N = length(x)))); head(data.pp); nrow(data.pp) # 60 - one measure of mean and sd per plant
...
dredge(m1.4) # Top model (by > 2AIC) has mean, dom_wild, percent_FAL, and no plant size
topmodel.pp <- glmmTMB(sd ~ mean + dom_wild + percent_FAL, data=data.pp, na.action = "na.fail")
...
topmodel.pp_nomean <- glmmTMB(sd ~ dom_wild + percent_FAL, data=data.pp, na.action = "na.fail")
...
AICctab(m3.1, m3.3) # A dummy variable is better - so, having reproductive stage doesn't add anything to the model
...
AICctab(m5.1, m5.2, m5.2_dummy, m5.3) # dummy random is the same as plant_pop; remove

##### B) Tweak fixedFX structure
m5.4 <- glmmTMB(sd ~ dom_wild*leaf_stage*mean + plant.volume_centered + percent_CAER + percent_FAL, data=data.ps)
dredge(m5.4) # top models
topmodel.ps <- glmmTMB(sd ~ dom_wild + leaf_stage + mean, data=data.ps) # top model
DHARMa::testResiduals(DHARMa::simulateResiduals(topmodel.ps))
check_collinearity(topmodel.ps)
...
##### A) Tweak ranFX structure NA
##### B) Tweak fixed FX structure NA
topmodel.ps_nomean <- glmmTMB(sd ~ dom_wild + leaf_stage + (1|plant_pop), data=data.ps)
...
##### B) Tweak fixed FX structure
m6.4 <- glmmTMB(mean ~ dom_wild*leaf_stage*plant.volume_centered + percent_CAER + percent_FAL + (1|plant_pop), data=data.ps, na.action = "na.fail")
...
ggplot(data=sla, aes(x=leaf_stage, y=percent.change, group=hypothesis)) +
  ...
  theme_bw()+
  geom_text(aes(label = sig.asterisk, y=max(sla$percent.change.upper)+25), size=5, color="gray32", position=position_dodge(width=.5), angle=30) +
